...
timetab = spark.createDataFrame(data, columns)

timetab.show()

timetab.createOrReplaceTempView('time_tab')

df = spark.sql('''
        SELECT sensorid, timestamp,
        (new - values) as values
        from (SELECT *,
            lead(values,1)
            over(partition by sensorid order by values) as new
            from time_tab)
